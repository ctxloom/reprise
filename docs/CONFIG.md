...
| Key | Type | Default | Meaning |
|---|---|---|---|
| `retriever` | string | `"landmark"` | Candidate-generation retriever (a string-keyed, plugin-extensible selector mirroring `[normalize] normalizer`). `"landmark"` is the §5.5.4 rare-peak constellation generator (the §7.4b rivalry winner). The bake-off's alternatives (minhash-lsh, winnowing, sourcerer-rare) implement the same `matchtree::Retriever` trait bench-side. Post-fingerprint → never in the cache key. |
