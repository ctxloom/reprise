...
| `normalizer` | string | `"ir"` | Which normalizer produces the canonical tree fed to matching. `"ir"` is the `src/frontend` + `src/ir` canonical IR — **the default** since the §8 switchover. `"historical"` is the per-language `src/lang` profile path, still supported and selectable. A **bounded** selector, unlike `[retrieval] retriever`: an unknown value (a typo like `"irr"`) fails at config load, so it can never silently fall through to the historical path. Languages without an IR frontend (TypeScript, Kotlin) fall back to the historical path even under `"ir"`. C is the converse — it is **IR-frontend-only**, so `"historical"` **refuses a scan** containing `.c`/`.h` files with an error naming the first one (select `"ir"`, or exclude those files); it never skips them silently. The IR trees are ~18% more compact, which is why the size and vote floors have per-normalizer `_ir` twins. This value enters the extraction cache key. |
...
| Key | Type | Default | Meaning |
|---|---|---|---|
| `retriever` | string | `"landmark"` | Candidate-generation retriever. **`"landmark"` is the only accepted value** — the §5.5.4 rare-peak constellation generator (the §7.4b rivalry winner). Validated at load: anything else is an error naming the accepted set, never a silent fallback. The bake-off's rivals (minhash-lsh, winnowing, sourcerer-rare) implement the same `matchtree::Retriever` trait but exist only bench-side, in `examples/bakeoff.rs`; they are not selectable here, because a key that accepts a name it then ignores is a config that lies. The key stays a string so a future retriever can register without a schema break. Post-fingerprint → never in the cache key. |
...
| `landmark_coverage_min` | float | `0.05` | The `coverage` filter's threshold (docs/substantiality-metric.md §0.3): retain a landmark candidate only when its shared constellation is ≥ this fraction of the smaller unit's landmark set. `0` disables the filter even when `coverage` is listed. Landmark-candidacy-scoped, so recall-safe. |
| `filters` | list | `["coverage", "offset-histogram", "h-tree"]` | Ordered near-tier FILTER cascade run **before** `anti_unify`. Each name is a pure predicate on a candidate pair; a pair must pass ALL to reach `anti_unify`. The cascade is a conjunction, so **order is result-invariant** — it only sets short-circuit cost. Filters: `coverage` (§0.3, landmark-candidacy-scoped — runs in the retriever phase), `offset-histogram` (Shazam Δoffset, §5.6), `h-tree` (H-tree-verify Δdepth, §0.5; reads the same shared evidence as `offset-histogram`, so it adds no extra walk). Unknown names error at load. |

**Why `anti_unify` is NOT in `filters`.** `anti_unify` is the **fixed terminal verify**, deliberately kept out of the list, for two reasons: (1) it is a *producer*, not a predicate — it yields the match itself (the AU template + divergence), not a pass/fail, so keeping it out lets `filters` stay homogeneous (pure predicates, no producer/filter type-union, and it can't be misordered); (2) it is the **dominant per-pair cost** — `au::anti_unify` recursively aligns the two units' trees via an O(n·m) Needleman-Wunsch DP, orders of magnitude heavier than the cheap filters (hash-set intersections for `coverage`, one-vote-per-bin histograms for `offset-histogram`/`h-tree`). It runs **once, last, only on filter-survivors**; the whole filter cascade exists to minimize how many pairs reach it (coverage ≈ −44%, h-tree ≈ −48% of the flood). `h-tree` was promoted into the default list after a whole-repo + inline-on gate proved `verified_pairs` byte-identical to the list without it (zero recall loss).
