...
**Rev 11 (2026-07-12, post-implementation correction):** The landmark layer's rarity gate — layer 4's `df ≤ rare_cap` peak admission test, added in Rev 10 as config — is **deleted, not tuned** (DECISIONS.md D51): measured inert as shipped (3 of 754,254 hashes rejected on a real corpus) and, even fixed to bind on the inventory it filters, removes zero junk groups at any threshold while costing real small clone families. Every offset-sorted subtree peak is now admitted into the constellation. The subtree hash set (§5.5 item 2) accordingly loses its remaining retrieval-adjacent role — it is no longer any gate's document-frequency substrate, only a persisted digest field. Affected: §5.5, §9.
...
2. **Subtree hash set:** the deduplicated set of Merkle hashes of all subtrees above a small size floor (`bag_min_subtree_tokens`, default 6). **This is not a retrieval layer.** It was specified as one — bag-Jaccard over these hashes as "the primary near-miss retrieval signal" — and was built that way; measurement retired it (§7.4(b), DECISIONS.md D48): its candidate set was a strict subset of the landmark layer's on every corpus, and it verified zero pairs landmark did not also propose. It briefly served as the landmark layer's rarity-gate document-frequency substrate; that gate is also deleted (DECISIONS.md D51 — measured inert, and unable to remove junk groups even fixed to bind). The set is now only a persisted digest field, consumed by the retrieval bake-off's comparison retrievers, not by the shipped landmark retriever.
...
bag_min_subtree_tokens = 6     # subtree-hash-set floor; NOT a retrieval threshold (§5.5.2) —
                               #   the set is a persisted digest field only
...
landmark_fan_out      = 3         # peaks each offset-sorted peak is paired with (index size ~linear)
                                   #   every peak is admitted — no rarity gate (DECISIONS.md D51)
